...
**Только сервер** (без браузера): путь одного HTTP-запроса через Django в этом проекте — сверху вниз.
...
Цепочка **без** Django Channels: реалтайм пока только в планах (п. 1.1 и `use_cases.md`).
